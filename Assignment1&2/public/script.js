...
// Open static image in new tab
var openImageButton = document.getElementById('open_image');
...
openImageButton.addEventListener('click', function(){
	...
	imageNode.src = strDataURI;
    var imageDiv = document.createElement('div');
    imageDiv.appendChild(imageNode);
    var w = window.open('#showImage');
    //console.log(imageDiv.innerHTML);
    w.document.write(imageDiv.innerHTML);
    w.document.location = '#showImage';
    w.document.close(); 
})

// allow enter to submit
document.getElementById('username_field').addEventListener('keydown', function(e) {
    e = e || window.event;
    if (e.keyCode == 13) {
        e.preventDefault();
        registerUser();
...
// Save image
var saveImageButton = document.getElementById('save_image');
saveImageButton.addEventListener('click', function(){
    ...
    let datetime = 
        timeObj.getDate() + '.' 
        + (timeObj.getMonth() + 1) + '.' 
        + timeObj.getFullYear() + "_" 
        + timeObj.getHours() + ':'
        + ((timeObj.getMinutes() < 10) ? ("0" + timeObj.getMinutes()) : (timeObj.getMinutes())) + ':' 
